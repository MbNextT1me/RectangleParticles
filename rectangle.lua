...
function Rectangle:onClick(ps, x, y)
    if x >= self.location.x and x <= self.location.x + self.width and y >= self.location.y and y <= self.location.y + self.height then
        ...
        local topRight = Vector:create(self.location.x + self.width, self.location.y)
        local bottomLeft= Vector:create(self.location.x, self.location.y  + self.height)
        local bottomRight= Vector:create(self.location.x + self.width, self.location.y + self.height)
